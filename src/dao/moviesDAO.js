...
export default class MoviesDAO {
  ...
  static async getMoviesByCountry(countries) {
    ...
    */
    ...
    try {
      ...
      const countri = Array.isArray(countries)
        ? countries
        : countries.split(", ")

      const project = { title: 1, _id: 1 }

      cursor = await movies.find(
        {
          countries: { $in: countri },
        },
        { projection: project },
      )
    } catch (e) {
  ...
  static async facetedSearch({
    ...
    */
    ...
    // const stag = []
    // stag.push(limitStage)
    // stag.push(skipStage)
    // stag.push(facetStage)
  ...
  static async getMovies({
    ...
    */
    ...
    // } else if (page === 1) {
    //   skips = moviesPerPage * page
    //   displayCursor = cursor.skip(skips).limit(moviesPerPage)
    // } else if (page === 2) {
    //   skips = moviesPerPage * page
    //   d
    // }

    const skips = moviesPerPage * page
    displayCursor = cursor.limit(moviesPerPage).skip(skips)
